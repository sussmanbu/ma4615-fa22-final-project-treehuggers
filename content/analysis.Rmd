...
full_join(parks_data_by_county, hospitalizations_white, by = c("county" = "county_name"))

CA_county_asthma_parks <- CA_county_asthma_parks %>%
full_join(hospitalizations_poc, by = c("county" = "county_name"))

CA_county_asthma_parks <- CA_county_asthma_parks %>%
full_join(hospitalizations_hispanic, by = c("county" = "county_name"))

CA_county_asthma_parks <- CA_county_asthma_parks %>%
full_join(hospitalizations_asian_pi, by = c("county" = "county_name"))

# as the AI/AN category values were all 0 or NA, we decided it was not useful to keep analyzing that information

CA_county_asthma_parks <- CA_county_asthma_parks %>%
full_join(hospitalizations_black, by = c("county" = "county_name"))

CA_county_asthma_parks <- CA_county_asthma_parks %>%
full_join(hospitalizations_total, by = c("county" = "county_name"))
...
```{r}
ggplotly(sums %>% ggplot(aes(x = total_open_park, y = number_hospitalizations, col = county_name)) + geom_jitter() + labs(x = "Open Parks", y = "Hospitalizations", col = "County Name"))
```
