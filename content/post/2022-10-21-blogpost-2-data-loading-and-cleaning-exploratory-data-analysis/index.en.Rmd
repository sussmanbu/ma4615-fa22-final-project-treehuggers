...
library(tidyverse)
parks <- read_csv("nanda_parks_tract_2018_01P.csv")
...
```{r echo=TRUE, plot2 echo=TRUE}
asthma_ca <- read_csv("asthma-hospitalization-rates-by-county-2015_2019.csv")

asthma_2018 <- asthma_ca %>% filter(YEAR == "2018")
asthma_2018 %>% filter(COUNTY == "California") %>% 
  group_by(`AGE GROUP`) %>% ggplot(aes(x = `AGE GROUP`, 
                                       y = `NUMBER OF HOSPITALIZATIONS`, 
                                       fill = `AGE GROUP`)) 
+ geom_col() + labs(x = "AGE GROUP", y = "HOSPITALIZATIONS")
```
