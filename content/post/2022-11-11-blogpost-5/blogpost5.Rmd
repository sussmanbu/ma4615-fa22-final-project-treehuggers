...
Our initial findings are that we have some outliers that we originally suspected were very large counties, specifically Los Angeles. Through plots, we can see that the outliers in the asthma datasets are both Los Angeles, which makes sense given what we previously noted about the county’s much denser population. Based on this plot, we can see that Los Angeles has the highest amount of hospitalizations by far compared to the other counties.

```{r}
asthmaCA_race_ethnicity %>% filter(county_name != "California", strata == "Total population", !is.na(number_hospitalizations)) %>% ggplot(aes(x = county_name, y = number_hospitalizations)) + geom_jitter() + labs(x = "County Name", y = "Number of Hospitalizations", title = "Total Number of Asthma Hospitalizations for California Counties in 2018") 
```
